export const CREATE_USER = 'CREATE_USER';
export const DUPLICATE_CHECK = 'DUPLICATE_CHECK';
export const LOGIN_USER = 'LOGIN_USER';
export const LOGOUT_USER = 'LOGOUT_USER';
export const UPDATE_USER_PASSWORD = 'UPDATE_USER_PASSWORD';
export const UPDATE_USER_NAME = 'UPDATE_USER_NAME';
export const CHECK_TOKEN = 'CHECK_TOKEN';
export const USE_REFRESH_TOKEN = 'USE_REFRESH_TOKEN';
export const REMOVE_USER = 'REMOVE_USER';
export const SEND_EMAIL_RESET_PASSWORD = 'SEND_EMAIL_RESET_PASSWORD';
export const RESET_PASSWORD = 'RESET_PASSWORD';
export const CAT_INFINITY = 'CAT_INFINITY';
export const DOG_INFINITY = 'DOG_INFINITY';
export const ADD_LIKE_IMAGES = 'ADD_LIKE_IMAGES';
export const REMOVE_LIKE_IMAGES = 'REMOVE_LIKE_IMAGES';
export const FETCH_LIKE_IMAGES = 'FETCH_LIKE_IMAGES';
export const CREATE_DIARY = 'CREATE_DIARY';
export const FETCH_DIARY = 'FETCH_DIARY';
export const GET_MY_DIARY = 'GET_MY_DIARY';
export const UPDATE_DIARY = 'UPDATE_DIARY';
export const REMOVE_DIARY = 'REMOVE_DIARY';
